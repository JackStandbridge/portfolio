...
const extensions = ['jpg', 'jpeg', 'png', 'gif', 'webp', 'tif', 'tiff', 'bmp'];
		...
		const result = data.gallery_data.items.map(async (item) => {
			try {
				const promises = await Promise.all(
					extensions.map((ext) =>
						fetch(`https://i.redd.it/${item.media_id}.${ext}`)
					)
				);

				const accessibleImage = promises.find(
					(promise) => promise.status < 300
				);
